...
- This package is superseded by
  [ccmux-state](https://github.com/wuwenrui555/ccmux-state). README
  carries a deprecation banner and the pyproject classifier moves
  from `Development Status :: 3 - Alpha` to
  `Development Status :: 7 - Inactive`. Three pane primitives
  (`has_input_chrome`, `parse_status_line`, `capture_pane`) live on
  inside ccmux-state with attribution; the rest of this package's
  API has no successor (claude-tap's
  `permission_request.tool_name` field replaces it). No further
  releases are planned beyond this one.
